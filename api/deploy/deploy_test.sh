...
# Stubs ssh/rsync/curl/go/npm/npx/systemctl/sleep on PATH ahead of the real
...
cat > "$STUBS/go" <<'EOF'
#!/bin/bash
# Fake build: write the new-release marker to the -o output path.
out=""
prev=""
for a in "$@"; do
  [ "$prev" = "-o" ] && out="$a"
...
setup_case() { # case-name
  ...
  mkdir -p "$CLONE/api" "$CLONE/web" "$REMOTE_ROOT"
